...
async function getScopeAtPos(offset: number, text: string) {
    ...
    return new Promise((resolve: (tag: IScope | boolean) => any) => {
            ...
            onOpenTag(event) {
                ...
                for (const attribute of attributes) {
                    ...
                    if (!attribute.argument) {
                        ...
                        console.log(`Looking in Attribute's Argument: ${
                            text.slice(attribute.argument.pos + 1, attribute.argument.endPos)
                        }`);
...
async function findDefinitionForTag(document: TextDocument, { tagName }: IScope): Promise<Definition> {
    const {
        template = false,
        renderer = false,
        taglibId,
...
async function findDefinitionForAttrName(document: TextDocument, { tagName, data: attrName }: IScope):
    Promise<Definition> {
...
async function findDefinitionForAttrValue(document: TextDocument, { data: attrValue }: IScope): Promise<Definition> {
...
export default async function onDefinition(positionParams: TextDocumentPositionParams, mls: MLS): Promise<Definition> {
    ...
    const scopeAtPos = await getScopeAtPos(offset, doc.getText()) as IScope;
    ...
    // ATTR_NAME: Return the marko.json file if it exists,
    // and otherwise go to the first usage of input.ATTR_NAME (or all of them)

    // ATTR_VALUE: Check if this is a handler to the ATTR_NAME
    // and return the definition of this handler either in the template or in the component.json
}
